...
plot_combined <- function(data1, data2, data3, hues=c("black","blue","green")) {
  plot(data1[,1], data1[,2], type = 'b', pch = 19, lty = 1, col = hues[1], ylim = range(c(data1[,2], data2[,2], data3[,2])), xlab = "Generation", ylab = "Fitness", main = "Best Fitness Values")
  ...
  legend("topleft", legend = c("GA1 crossover=0.2", "GA2 crossover=0.5", "GA3 crossover=0.8"), col = hues, pch = 19, lty = 1, cex = 0.8)  # Adjusting position and adding crossover info, reducing font size with cex
}
