getData <- function() {
  distMatrix <- read.csv("E:/amazon_last_mile_delivery_optimization/dataset/newdistance_dataset.csv", row.names=1)
...
plotTSPSolution<-function(solution){
  ...
  mds <- cmdscale("E:/amazon_last_mile_delivery_optimization/dataset/newdistance_dataset.csv")
  x <- mds[, 1]
  y <- -mds[, 2]
  ...
  text(x, y, labels(newdistance_dataset), cex=0.8)
}